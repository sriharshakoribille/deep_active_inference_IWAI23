...
import src_torch.torchutils as tfu
import src_torch.mcts_torch as mcts
...
model = ActiveInferenceModel(s_dim=s_dim, pi_dim=pi_dim, gamma=1.0, beta_s=1.0, beta_o=1.0, colour_channels=1, resolution=64, device=device)
...
with torch.no_grad():
    ...
    qs0_mean, qs0_logvar = model.model_down.encoder(torch.Tensor(o0).permute(0,3,1,2).to(device))
    ...
    while t < args.duration:
        ...
        if len(executing_steps) == 0:
            ...
            if args.method == 'habit':
                ...
                mcts_path, repeats_done, states_explored, all_paths, all_paths_G = mcts.active_inference_mcts(model=model, frame=o_single, params=params, o_shape=(1,64,64), device=device)
